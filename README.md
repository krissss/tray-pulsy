...
**TrayPulsy** is a lightweight, open-source macOS menu bar application that displays an animated cat (or other creature!) whose running speed scales with your system usage (CPU or Memory). The harder your Mac works, the faster it runs!
...
| ⚡ **Speed Sources** | Animation driven by **CPU**, **GPU**, **Memory**, or **Disk** usage |
| 🎯 **FPS Limits** | Cap at 40 / 30 / 20 / 10 fps to save battery |
| 🌓 **Theme Support** | System / Light / Dark mode with automatic icon recoloring |
| 📊 **Live System Info** | CPU%, Memory (GB), Disk (GB), Network speed in menu |
| 🔢 **Menu Bar Text** | Optional metric% display next to icon — auto-follows speed source |
...
├── StatusBarController.swift # NSStatusItem owner, settings window, accessibility
├── CatAnimator.swift      # Timer-driven frame animation (RunLoop.common)
├── SkinManager.swift      # Skin loading, theme recoloring (CIFilter)
├── SystemMonitor.swift    # CPU/Memory/Disk/GPU via kernel APIs
├── SettingsStore.swift    # UserDefaults persistence (Defaults library)
└── Resources/             # PNG sprite frames (cat, horse, parrot, frog, snail)
```
...
**Prerequisites:** Xcode 26+ or Swift 6.3+, macOS 26+

```bash
git clone https://github.com/krissss/TrayPulsy.git
cd TrayPulsy
